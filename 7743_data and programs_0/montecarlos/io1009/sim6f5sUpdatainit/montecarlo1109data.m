...
while ns<Nsim+1
    ...
    %GETTING INITIAL PARAMETERS FOR REDUCED FORM AND 
    Zred=[ones(Nm*T*S*(Nf+1),1) Z2(:,2) (Z2(:,2)./10).^2 LFirm2 LNFirm2 (LNFirm2./5).^2 Z2(:,3) Z2(:,2).*Z2(:,3)./10 LFirm2.*Z2(:,3) LNFirm2.*Z2(:,3)./10];

    [btemp]=fminunc('wlogitd',zeros(size(Zred,2),1),o1,Firm2,Zred,0,ones(Nm*(Nf+1)*T*S,1));
    p=uprobmred(btemp,Nf,S,Xn);
    
    [fv,bigp]=fv1109(p,trans,Nf,Xn,S,bine);
    
    
    %MATHING THE FUTURE VALUE TERMS TO THE DATA THE INITIAL CCPS AND FUTURE VALUE TERMS
    ...
    while cond==0; 
        ...
        %UPDATING THE CCP'S WITH THE DATA
               
            [btemp]=fminunc('wlogitd',btemp,o1,Firm2,Zred,0,PType2);
    ...
    save bdata Bccp Byccp Tccp Iccp Lccp Binit
