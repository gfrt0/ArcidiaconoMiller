...
while ns<Nsim+1
    ...
    Z2=kron(ones((Nf+1),1),[ones(Nm*T,1) X-1 S2]);

    Y2=reshape(Y,Nm*T,1);
    
    ZZ2=[ones(Nm*T,1) reshape(NFirm,Nm*T,1) X-1 S2];
    ...
    Zred=[ones(Nm*T*(Nf+1),1) Z2(:,2) (Z2(:,2)./10).^2 LFirm2 LNFirm2 (LNFirm2./5).^2 Z2(:,3) Z2(:,2).*Z2(:,3)./10 LFirm2.*Z2(:,3) LNFirm2.*Z2(:,3)./10];
