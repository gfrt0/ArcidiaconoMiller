...
alphaf=[alpha(1:2);log(alpha(4))-log(1-alpha(4))];
alphac=[alpha(1:2);alpha(4)];
...
for MC=1:50;
    ...
    tic
    %bf=0;
    [bf]=fminunc('likebusML4',alphaf,o1,[Y],N,T,[X],[Zstate],[Xstate],xtran,tbin,zbin,xbin,xval,Z);
    tf1=toc
    ...
    xx=[ones(N*T,1) x2 z2 x2.*z2 x2.*x2 z2.*z2];
    xx=[xx ((t2)*ones(1,6)).*xx ((t2.*t2)*ones(1,6)).*xx];
