...
% cd('7743_data and programs_0/montecarlos/busprograms/hetero10ns')
rng(44042, 'twister');
...
[bccp]=fminunc('wlogit',[bccp],o1,y2,[xccp fvt1 td(:, 1:T2-2)],PType)

while cond==0;

    pi = round( mean(reshape(PType, N * T2, 2)), 2);
...
for MC=1:50;
    ...
    while cond==0;
        ...
        if mod(j, 10) == 0 
            pi = round( mean(reshape(PType, N * T2, 2)), 2);
            fprintf('Monte Carlo %d, EM iteration %d. Pi: %d. Structural parameters: ', MC, j, pi(1))
            fprintf('%.2f ', bccp(1:4)')
            fprintf('\n')
        end     
...
for i = 1:size(Bccp(:, 1:4), 2)
    subplot(2, 2, i); % or use subplot(rows, cols, i) as needed
    histogram(Bccp(:, i));
    hold on; % Allows plotting on top of histogram
    xline(alpha(i), 'r--', 'LineWidth', 2); % Red dashed line at mean
